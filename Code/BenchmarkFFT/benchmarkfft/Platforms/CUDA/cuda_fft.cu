...
__host__ int cuda_validate(int n)
{
    ...
    size_t buffer_size = cu_batch_size(n) * sizeof(cpx);
...
__host__ int cuda_2d_validate(int n, bool write_img)
{
    ...
    int res = cuda_compare_result(host_buffer, ref, dev_in, size, cu_batch_size(n * n));
...
__device__ __inline void cuda_partial(cpx *in, cpx *out, cpx *shared, unsigned int in_high, unsigned int offset, float local_angle, int steps_left, int leading_bits, float scalar)
{
    ...
    *in_u = in[in_high];
    ...
    out[BIT_REVERSE(threadIdx.x + offset, leading_bits)] = cpx{ in_l->x * scalar, in_l->y * scalar };
    out[BIT_REVERSE(in_high + offset, leading_bits)] = cpx{ in_u->x * scalar, in_u->y * scalar };
}
