...
int dx_2d_validate(const int n)
{
    ...
    write_image("DirectX", "transposed", out, n);
    swap_io(&args);
...
__inline void dx_set_trans_args(dx_args *a)
{
    a->context->CSSetShaderResources(0, 1, &a->view_nullptr);
    a->context->CSSetUnorderedAccessViews(0, 1, &a->buf_output_uav, &a->init_cnts);
    a->context->CSSetShaderResources(0, 1, &a->buf_input_srv);
    a->context->CSSetShader(a->cs_transpose, nullptr, 0);
}
...
{
    if (n > 256) {
        int width = n > TILE_DIM ? (n / TILE_DIM) : 1;
        /*
        dx_fft_2d_helper(dir, args, n);
        swap_io(args);
        */
        dx_set_trans_args(args);
        ...
        */
        ...
        const float global_angle = dir * (M_2_PI / n);
        dx_set_local_args(args, global_angle, global_angle, steps_left, 0, 0, scalar, 0, n, false);
        args->context->Dispatch(args->n_groups.x, args->n_groups.y, 1);
        swap_io(args);
        dx_set_local_args(args, global_angle, global_angle, steps_left, 0, 0, scalar, 0, n, true);
        args->context->Dispatch(args->n_groups.x, args->n_groups.y, 1);
    }
}