...
void MyFFTW::runPerformance(const int n)
{
    ...
    if (dim1) {
        p = fftwf_plan_dft_1d(n, fftw_in, fftw_out, FFTW_FORWARD, FFTW_PATIENT); // FFTW_MEASURE, FFTW_PATIENT, FFTW_EXHAUSTIVE 
        ...
        p = fftwf_plan_dft_2d(n, n, fftw_in, fftw_out, FFTW_FORWARD, FFTW_PATIENT);
        for (int i = 0; i < number_of_tests; ++i) {
            start_timer();
            fftwf_execute(p);
            measures[i] = stop_timer();
