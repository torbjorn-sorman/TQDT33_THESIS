...
The experiments are tuned on a few parameters, the number of \glspl{thread} per \gls{block} and how large the tile dimensions are in the transpose \gls{kernel}, see chapter \ref{cha:implementation} and table \ref{tab:threads-per-block}.
...
% Performance optimizations were not portable all the time, some issues solved in \{CU} colud lead to reduced performance in another technology. A good practice was to improve the algorithm in general instead of attempting specialization in each platform, however some variables were tweaked since they had such great impact on performance. 
...
The graphic cards used in this thesis are at least one generation old compared to the current latest graphic cards. There would be interesting to see if the cards have the same differences in later generations and to see how much have been improved over the generations. It is likely that the software drivers are differently optimized towards the newer graphic cards.
