...
\begin{enumerate}
	\item \texttt{clGetPlatformIDs}\ldots
	\item \texttt{clGetDeviceIDs}\ldots
	\item \texttt{clCreateContext}\ldots
	\item \texttt{clCreateCommandQueue}\ldots
	\item \texttt{clCreateBuffer}\ldots
	\item \texttt{clEnqueueWriteBuffer}\ldots
	\item \texttt{clCreateProgramWithSource}\ldots
	\item \texttt{clBuildProgram}\ldots
	\item \texttt{clCreateKernel}\ldots
\end{enumerate}
...
Kernel execution share more similarities

\begin{table}
	\begin{tabular}{|l|l|}
		\hline
		CUDA & \texttt{cuda\_kernel$<<<$ blocks, threads $>>>$(Arguments)} \\ \hline
	\end{tabular}
\end{table}
