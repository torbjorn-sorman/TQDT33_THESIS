...
All tests executed on the GPU utilize some form of event timestamps supplied by the technology used. See figure \ref{fig:code:timestamps} for specific method per technology.

\begin{figure}[h!]
	\centering
		\begin{framed}
			\subfloat[CUDA event]{
				\label{fig:code:timestamp:cu}
				\includestandalone[width=\textwidth]{code/timestamps/cu}
	    	}
			\subfloat[CUDA event]{
				\label{fig:code:timestamp:ocl}
				\includestandalone[width=\textwidth]{code/timestamps/cu}
    		}
			\subfloat[CUDA event]{
				\label{fig:code:timestamp:dx}
				\includestandalone[width=\textwidth]{code/timestamps/cu}
	    	}
			\subfloat[CUDA event]{
				\label{fig:code:timestamp:gl}
				\includestandalone[width=\textwidth]{code/timestamps/cu}
	    	}
			\subfloat[CUDA event]{
				\label{fig:code:timestamp:omp}
				\includestandalone[width=\textwidth]{code/timestamps/cu}
	    	}
		\end{framed}
	\caption{ Method to extract timestamps for kernel events for the different technologies. }
	\label{fig:code:timestamps}
\end{figure}
