...
\begin{tikzpicture}[
	...
	\node[add, name=N-1-0, pin={[pin edge={-latex',black}]right:$X(0)$}] at (1,-0) {${+}$};
	...
	\path (N-0-0) edge[->] (N-1-0);	
	\path (N-0-1) edge[->] (N-1-1);
	\path (N-1-1) edge[->] (N-2-1);
      
    % Connect nodes
    \foreach \src / \dst in {	0/0, 0/1, 1/0, 1/1}
       \path (N-0-\src.east) edge[->] (N-1-\dst.west);
